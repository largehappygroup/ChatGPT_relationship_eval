...
def ranking_prompt(content_list, score_info_list, temperature_value, num_comments, topics):

    usr_prompt = "I will give you a description of a relationship problem and " + str(num_comments) + " advices on a the problem. Can you help me rank the reliability of the advices? You do not need to provide the reasonings for your ranking. Show me the results in the form of number series separated by comma, for instance, \"3,1,2,5,4\" if option 3 is the best comment and option 4 is the worst. Make sure that all" + str(num_comments) + "numbers are present in your response. To help your task, you are told that this problem is about the following topics: " + str(topics) + ".The description of the problem is: "
...
def path_selection():
    ...
    home_path = "/home/haonan/interesting_16_finer/"
...
def individual_store_info(post, content_list, individual_cv, store_path, selected_comment_indices, gpt_ranking):
    individual_dict = {'cv': individual_cv, 'gpt_ranking': gpt_ranking, 'description': content_list[0]}
...
def info_extraction(post, folder_path, temperature_value, num_comments, store_path, topics):
    global cnt_posts, cnt_errors, errored_posts
    global cumulated_cv, cumulated_controversial_comments, extreme_posts
    ...
    try:
        ...
        gpt_response, individual_cv, selected_comment_indices = ranking_prompt(content_list, score_info_list,
                                                                              temperature_value, num_comments, topics)

        if gpt_response is None:  # If unable to parse the response
            unresolved_store_info(post, store_path, gpt_response)
        else:
            gpt_ranking = gpt_response
...
with open("/home/haonan/labeled_posts.json", "r") as infile:
    json_str = json.load(infile)
    for post in json_str:
        labels = json_str[post]
        info_extraction(post, home_path, temperature_value, num_comments, store_path, labels)
